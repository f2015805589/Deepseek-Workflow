...
- **No companion packages required**: the desktop is a plain shell over the web profile and depends only on harness packages every stock checkout ships (`@deepseek-ai/dsh`, `dsh-app-boot`, `dsh-host-webserver`, `dsh-settings`). Earlier revisions shipped desktop-owned product plugins (compaction setting, conversation edit, fs revert, plugin manager); those package sources were lost and are no longer referenced as separate packages anywhere in this repo — nothing in the overlay or the dependency graph names them. The custom-plugin manager, compaction-threshold control, Cursor-style in-place message editing (same-session, no fork), and per-turn file revert are shipped again as a **custom plugin** (`plugins/dsh-client-ui-desktop-plugins`, enabled by default via `plugins/plugins.json`).
...
The desktop renders the composed web profile exactly as the browser does — the window is a hardened shell over the real Web GUI, with no desktop-owned product plugins. Earlier revisions shipped four companion plugins (edit & resend, per-turn file revert, an auto-compaction threshold control, and a custom-plugin manager tab); their sources are lost and this repo no longer ships them as separate packages, so the surface overlay carries only the window-facing config rows (`webserver`, `web-runtime`).

> Note: all four product capabilities are provided again by the committed **custom plugin** `plugins/dsh-client-ui-desktop-plugins` (enabled by default): the 自定义插件 manager tab in Settings → 插件, the compaction-threshold control beside Full access, Cursor-style **in-place message editing** (click a sent bubble → boxed editor with 取消/发送 inside; blur commits the edit into the SAME conversation's model surface — later requests read the edit, the old reply stays; 发送 rewinds the conversation to before the edited turn, archives the parent so no new dialog appears, and resends — the old reply is deleted and the model answers the corrected message; a first-turn message falls back to a fresh blank session), the per-turn 撤销修改 file-revert action, and a right-side **文件改动** dock (pending file changes top to bottom with green +N / red -N line stats, per-file 撤销/保存, and top-level 一键撤销/一键保存). The plugin's host half implements the edit/rewind IPC (`dsh-desktop:conversation-edit:*`), the fs-revert IPC (`dsh-desktop:fs-revert:*`), and the file-changes IPC (`dsh-desktop:fs-changes:*`).
