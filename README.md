...
> Note: all four product capabilities are provided again by the committed **custom plugin** `plugins/dsh-client-ui-desktop-plugins` (enabled by default): the 自定义插件 manager tab in Settings → 插件, the compaction-threshold control beside Full access, Cursor-style **in-place message editing** (click a sent bubble → editor with 取消/发送; clicking elsewhere commits the edit into the SAME conversation — the host half appends a `surfaceOp: replace` event so later requests read the edited wording; no fork, no new dialog), and the per-turn 撤销修改 file-revert action. The plugin's host half implements the edit IPC (`dsh-desktop:conversation-edit:apply`) and the fs-revert IPC (`dsh-desktop:fs-revert:*`).
